...
public class SoyoAccessibilityService extends AccessibilityService {
    ...
    private static final String PREF_RECENT_HANDLED = "recent_handled_rows_v24";
    ...
    private String carriedRecommendationKey;
    private long carriedRecommendationAt;
    private boolean carriedRecommendationVip = false;
    private String pendingName;
    private String pendingMessage;
    private String pendingRecommendationKey;
    private boolean sendTapIssued = false;
    ...
    private static final long HANDLED_TTL_MS = 6L * 60L * 60L * 1000L;
    private static final int MAX_PERSISTED_HANDLED = 240;
    ...
    // Satır bazlı anahtar kullanılır. Böylece aynı isimli iki farklı kullanıcı birbirini
    // gereksiz yere engellemez; servis aç/kapa olduğunda da son işlenen satırlar korunur.
    private final Set<String> handledRecommendationKeys = new HashSet<>();
    ...
    protected void onServiceConnected() {
        ...
        carriedRecommendationKey = null;
        pendingRecommendationKey = null;
        loadRecentHandledRows();
        mainHandler.postDelayed(automationWatchdog, WATCHDOG_INTERVAL_MS);
        requestScan(120L);
        Toast.makeText(this, "SOYO tam otomatik hazır.", Toast.LENGTH_SHORT).show();
    }
    ...
    private void maybeOpenRecommendation(AccessibilityNodeInfo root) {
        // Geri dönüş veya önceki sohbetin gönderim işlemi tamamlanmadan yeni sohbet açma.
        // SOYO mesajdan sonra listeye kendi dönerse eski sürüm sıradaki sohbeti çok erken açıp
        // autoSendScheduled=true state'ini yeni sohbete taşıyabiliyordu; bu da takılmanın ana
        // nedenlerinden biriydi.
        if (returningToRecommendations || autoSendScheduled || !TextUtils.isEmpty(pendingName)) return;
        ...
        try {
            ...
            automationEpoch++;
            final long openEpoch = automationEpoch;
            carriedRecommendationName = target.name;
            carriedRecommendationKey = target.key;
            ...
            if (!clicked) {
                ...
                // Flutter bazen sohbet geçişinde accessibility event üretmiyor. Elle kaydırma
                // gerektirmesin diye geçişten sonra iki kontrollü tarama daha iste.
                requestScan(140L);
                mainHandler.postDelayed(() -> {
                    if (openEpoch == automationEpoch) requestScan(0L);
                }, 520L);
            }
    ...
    private RecommendationTarget findFirstRecommendationTarget(List<AccessibilityNodeInfo> nodes) {
        ...
        for (AccessibilityNodeInfo chatNode : nodes) {
            ...
            String rowKey = buildRecommendationKey(nodes, chatRect, rowName.name);
            if (handledRecommendationKeys.contains(rowKey)) continue;
    ...
    private String buildRecommendationKey(List<AccessibilityNodeInfo> nodes, Rect chatRect, String name) {
        if (nodes == null || chatRect == null) return "name:" + nameKey(name);
        int tolerance = Math.max(dp(36), Math.min(dp(52), chatRect.height() + dp(16)));
        ...
        for (AccessibilityNodeInfo node : nodes) {
            ...
            String token = Normalizer.normalize(
                    (nodeText(node) + " " + safe(node.getContentDescription())).trim(),
                    Normalizer.Form.NFKC).replaceAll("\\s+", " ").trim();
            if (token.isEmpty()) continue;
            String normalized = normalizedLower(token);
            if (isChatLabel(normalized)) continue;
            if (normalized.length() > 56) normalized = normalized.substring(0, 56);
            if (seen.add(normalized)) tokens.add(normalized);
        }
        java.util.Collections.sort(tokens);
        StringBuilder out = new StringBuilder("row:").append(nameKey(name));
        int count = 0;
        for (String token : tokens) {
            out.append('|').append(token);
            if (++count >= 10) break;
        }
        // Satırda isim dışında ayırt edici metadata yoksa aynı isimli iki kişiyi tek kişi
        // sanmamak için kaba ekran slotunu da ekle. Normal durumda metadata anahtarı stabildir.
        if (tokens.size() <= 1) {
            out.append("|slot:").append(chatRect.centerY() / Math.max(1, dp(40)));
        }
        return out.toString();
    }
    ...
    private void prepareChatMessage(AccessibilityNodeInfo root, AccessibilityNodeInfo editor) {
        ...
        // İsim bazlı 3.5 sn kilit kaldırıldı. O kilit, art arda aynı isimli farklı bir
        // sohbette event gelmeyince ekranın elle kaydırılmasına kadar akışı durduruyordu.
        // Bunun yerine yalnızca listeden taşınan gerçek satır anahtarını kontrol ediyoruz.
        if (isFullAutoEnabled() && !TextUtils.isEmpty(carriedRecommendationKey)
                && handledRecommendationKeys.contains(carriedRecommendationKey)) {
            recycleNode(editor);
        ...
        pendingRecommendationKey = carriedRecommendationKey;
        sendTapIssued = false;
        recycleNode(send);
    ...
    private void attemptAutomaticSend(String expectedName, long expectedEpoch, int attempt) {
        ...
        if (freshRoot != null && isSoyoRoot(freshRoot)) {
            ...
            if (freshEditor != null) {
                ...
                if (liveHeader != null && !samePersonName(expectedName, liveHeader.name)) {
                    ...
                    String liveRecommendationKey = pendingRecommendationKey;
                    automationEpoch++;
                    autoSendScheduled = false;
                    pendingName = null;
                    pendingMessage = null;
                    ...
                    carriedRecommendationKey = liveRecommendationKey;
        ...
        if (clicked) {
            // EN ÖNEMLİ DEĞİŞİKLİK: aynı sohbet için gönder düğmesine yalnızca bir kez bas.
            // Eski kod kanıt bulamazsa aynı düğmeye tekrar basıyor ve aynı mesajı iki kez
            // gönderebiliyordu. Tıklama kabul edildiği anda satırı işlenmiş sayıp geri dön.
            sendTapIssued = true;
            markHandled(expectedName);
            mainHandler.postDelayed(() -> finishAutomaticSend(expectedName, expectedEpoch, false), 220);
            return;
        }
    ...
    private void confirmAutomaticSend(String expectedName, long expectedEpoch, int attempt) {
        ...
        } else if (sendTapIssued) {
            // Bir gönder dokunuşu yapıldıysa ikinci kez asla dokunma. UI kanıtı kaçmış olsa
            // bile exactly-once davranışı daha güvenli; sıradaki sohbete geç.
            finishAutomaticSend(expectedName, expectedEpoch, false);
        } else {
            autoSendScheduled = false;
            mainHandler.postDelayed(() -> scheduleAutomaticSend(expectedName, expectedEpoch), 180);
        }
    }
    ...
    private boolean isFullAutoEnabled() {
        // v2.4: manuel onay modu kaldırıldı; erişilebilirlik servisi açıksa akış tam otomatiktir.
        return true;
    }
    ...
    private void markHandled(String name) {
        String key = !TextUtils.isEmpty(pendingRecommendationKey)
                ? pendingRecommendationKey
                : carriedRecommendationKey;
        if (TextUtils.isEmpty(key) && !TextUtils.isEmpty(name)) key = "name:" + nameKey(name);
        if (TextUtils.isEmpty(key)) return;
        handledRecommendationKeys.add(key);
        persistHandledRow(key);
    }

    private void loadRecentHandledRows() {
        handledRecommendationKeys.clear();
        Set<String> stored = getSharedPreferences(PREFS, MODE_PRIVATE)
                .getStringSet(PREF_RECENT_HANDLED, null);
        ...
        for (String entry : stored) {
            ...
            if (sep <= 0 || sep >= entry.length() - 2) { changed = true; continue; }
            try {
                long when = Long.parseLong(entry.substring(0, sep));
                String key = entry.substring(sep + 2);
                if (now - when <= HANDLED_TTL_MS && now >= when) {
                    handledRecommendationKeys.add(key);
                    cleaned.add(entry);
                } else {
                    changed = true;
        ...
        if (changed) {
            getSharedPreferences(PREFS, MODE_PRIVATE).edit()
                    .putStringSet(PREF_RECENT_HANDLED, cleaned).apply();
        }
    }

    private void persistHandledRow(String key) {
        if (TextUtils.isEmpty(key)) return;
        android.content.SharedPreferences prefs = getSharedPreferences(PREFS, MODE_PRIVATE);
        Set<String> old = prefs.getStringSet(PREF_RECENT_HANDLED, null);
        Set<String> entries = old == null ? new HashSet<>() : new HashSet<>(old);
        long now = System.currentTimeMillis();
        String sameSuffix = "::" + key;
        Set<String> toRemove = new HashSet<>();
        for (String entry : entries) {
            if (entry == null || entry.endsWith(sameSuffix) || isExpiredHandledEntry(entry, now)) {
                toRemove.add(entry);
            }
        }
        entries.removeAll(toRemove);
        entries.add(now + sameSuffix);
        while (entries.size() > MAX_PERSISTED_HANDLED) {
            String oldest = null;
            long oldestTime = Long.MAX_VALUE;
            for (String entry : entries) {
                int sep = entry == null ? -1 : entry.indexOf("::");
                if (sep <= 0) { oldest = entry; oldestTime = Long.MIN_VALUE; break; }
                try {
                    long when = Long.parseLong(entry.substring(0, sep));
                    if (when < oldestTime) { oldestTime = when; oldest = entry; }
                } catch (NumberFormatException e) { oldest = entry; oldestTime = Long.MIN_VALUE; break; }
            }
            if (oldest == null) break;
            entries.remove(oldest);
        }
        prefs.edit().putStringSet(PREF_RECENT_HANDLED, entries).apply();
    }

    private boolean isExpiredHandledEntry(String entry, long now) {
        if (entry == null) return true;
        int sep = entry.indexOf("::");
        if (sep <= 0) return true;
        try {
            long when = Long.parseLong(entry.substring(0, sep));
            return when > now || now - when > HANDLED_TTL_MS;
    ...
    private void hideApproval() {
        // v2.4'te manuel overlay yok. Bu yöntem yalnızca bekleyen otomasyon durumunu temizler.
        pendingName = null;
        pendingMessage = null;
        pendingRecommendationKey = null;
        sendTapIssued = false;
    ...
    private static class RecommendationTarget {
        ...
        RecommendationTarget(String name, String key, AccessibilityNodeInfo chatButton, Rect chatRect, int score, boolean vip) {
            this.name = name;
            this.key = key;
